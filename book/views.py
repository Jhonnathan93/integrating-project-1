...
def response(request):
    ...
    recommendations = _recommend_books(request.user if request.user.is_authenticated else None, books, topics, genres, details, request.POST.get("longitud", ""))
    if request.user.is_authenticated:
        history_create(user=request.user, books=books, topics=topics, genres=genres)
    return render(request, "response.html", {"respuesta": "", "libros": recommendations})


def _selected_values(post_data, fields):
    return [post_data[field].strip() for field in fields if post_data.get(field, "").strip()]
...
def _openai_recommendations(user, books, topics, genres, details, length):
    ...
    try:
        ...
        prompt = ("Recomienda al menos 10 libros en español. Responde solo como 'título - autor', "
        ...
        completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_tokens=900)
        return [item.strip() for item in completion.choices[0].message["content"].replace('"', "").split(";") if item.strip()]
    except Exception:
...
def markAsNotRecommended(request):
    ...
    title, author = (data.get("title") or "").strip(), (data.get("author") or "").strip()
