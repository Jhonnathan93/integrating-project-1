...
class BookSelectorsTests(TestCase):
    def test_books_recommended_orders_books_by_most_recent(self) -> None:
        ...
        Book.objects.filter(pk=older.pk).update(dateAdded=timezone.now() - timedelta(days=1))

        self.assertEqual(list(books_recommended()), [newer, older])

    def test_disliked_book_titles_returns_only_titles_for_the_requested_user(self) -> None:
        ...
        another_profile = UserInformation.objects.create(user=another_user, preferences="")
