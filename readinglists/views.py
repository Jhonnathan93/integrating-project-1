...
def add_to_reading_list(request):
    ...
    book, _ = Book.objects.get_or_create(
        ...
        author=author,
        defaults={
            "description": data.get("description") or "Sin descripción disponible",
            "buy_link": data.get("buyLink") or "",
            "cover": data.get("cover") or Book._meta.get_field("cover").default,
        },
    )
    try:
    ...
    return JsonResponse(
        {
            "message": "Libro agregado a Leer más tarde."
            if added
            else "El libro ya está en Leer más tarde."
        }
    )
    ...
    if request.method == "POST":
        ...
        if not title or not author:
            return _detail_response(
                request, reading_list, "El título y el autor son obligatorios."
            )
        ...
        if not book_data:
            return _detail_response(
                request,
                reading_list,
                "No fue posible encontrar un libro con esos datos.",
            )
        book, _ = Book.objects.get_or_create(
            title=book_data["title"], author=book_data["author"], defaults=book_data
        )
        try:
